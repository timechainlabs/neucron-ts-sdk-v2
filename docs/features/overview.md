...
| **Headers** | Auth and context headers the SDK attaches (`Authorization`, business/team IDs, app secret, …) |
...
| [Team](team.md) | `sdk.team` | Team invites and members |
...
Pass `businessId` in function options to scope the call to a business. The SDK sets the `X-Neucron-Business-ID` header.

### Team context

Pass `teamId` where supported (teams, Asset21, some member flows). The SDK sets `X-Neucron-Team-ID`.
